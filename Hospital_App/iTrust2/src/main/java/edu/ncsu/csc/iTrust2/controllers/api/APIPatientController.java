...
public class APIPatientController extends APIController {
    ...
    /**
     * This method will assign patient advocate to patient
     *
     * @param idP
     *            Patient ID
     * @param idPA
     *            Patient Advocate User Form
     * @return Success if the idPA is a patient advocate and it is on the
     *         personnel list. Otherwise, false
     */
    @PostMapping ( BASE_PATH + "/patient/{idP}" )
    @PreAuthorize ( "hasAnyRole('ROLE_ADMIN')" )
    public ResponseEntity assignPatientAdvocateToPatient ( @PathVariable ( "idP" ) final String idP,
            @RequestBody final PatientAdvocate idPA ) {

        // Retrieve saved patient from database
        final Patient patient = (Patient) patientService.findByName( idP );

        // If patient does not exist in the database, send error response
        if ( patient == null ) {
            return new ResponseEntity( errorResponse( "Invalid personnel " ), HttpStatus.BAD_REQUEST );
        }

        // Retrieve saved patient advocate from database
        final PatientAdvocate patientAd = (PatientAdvocate) personnelService.findById( idPA.getUsername() );

        // If the passed-in patient advocate doesn't have the PatientAdvocate
        // role, send error response
        if ( !patientAd.getRoles().contains( Role.ROLE_PATIENTADVOCATE ) ) {
            return new ResponseEntity( errorResponse( "Invalid personnel " ), HttpStatus.BAD_REQUEST );
        }

        // Add patient advocate to patient's associatoins
        patient.addPatientAdvocates( patientAd );

        // Persistently save patient to database
        patientService.save( patient );
    ...
    /**
     * This method will unassigns patient advocate to patient
     *
     * @param idPA
     *            Patient Advocate to unassigns
     * @param idP
     *            patient ID
     * @return Success if the idPA is a patient advocate and it is on the
     *         personnel list. Otherwise, false
     */
    @DeleteMapping ( BASE_PATH + "/patient/{idP}/{idPA}" )
    @PreAuthorize ( "hasAnyRole('ROLE_ADMIN')" )
    public ResponseEntity unassignPatientAdvocateToPatient ( @PathVariable ( "idP" ) final String idP,
            @PathVariable final String idPA ) {

        // Retrieve saved patient from database
        final Patient patientA = (Patient) patientService.findByName( idP );

        // Retrieve saved patient advocate from database
        final PatientAdvocate patientAd = (PatientAdvocate) personnelService.findByName( idPA );

        // If the patient advocate does not exist, send error response
        if ( null == patientAd ) {
            return new ResponseEntity( errorResponse( idPA + " does not exist." ), HttpStatus.NOT_FOUND );
        }

        // Delete patient advocate from patient's association
        patientA.deletePatientAdvocates( patientAd );
